...
check_version_exists() {
  ...
  for attempt in 1 2 3; do
    ...
    if response=$(curl -s --max-time 30 --retry 2 --retry-delay 1 \
      ...
      if [[ "$http_code" == "200" ]] && [[ -n "$response" ]]; then
        echo "  Successfully fetched versions (attempt $attempt)"
        break
        ...
        echo "  HTTP error $http_code on attempt $attempt"
        if [[ $attempt -eq 3 ]]; then
          echo "  Warning: Failed to fetch versions for $crate_name after 3 attempts" >&2
      ...
      if [[ $attempt -eq 3 ]]; then
        echo "  Warning: Network error fetching versions for $crate_name" >&2
        echo "  Assuming version exists to avoid false negatives" >&2
  ...
  if [[ -z "$response" ]]; then
    echo "  Warning: Empty response for $crate_name versions" >&2
    echo "  Assuming version exists to avoid false negatives" >&2
    return 0  # Assume version exists to avoid false negatives
  fi
...
get_latest_version() {
  ...
  echo "  Fetching crate info for $crate_name..."
  ...
  for attempt in 1 2 3; do
    ...
    if response=$(curl -s --max-time 30 --retry 2 --retry-delay 1 \
      ...
      if [[ "$http_code" == "200" ]] && [[ -n "$response" ]]; then
        echo "  Successfully fetched crate info (attempt $attempt)"
        break
      elif [[ "$http_code" == "429" ]]; then
        echo "  Rate limited, waiting before retry..."
        sleep $((attempt * 2))
        ...
        echo "  HTTP error $http_code on attempt $attempt"
        if [[ $attempt -eq 3 ]]; then
          echo "  Error: Failed to fetch crate info for $crate_name after 3 attempts" >&2
          return 1
        fi
        continue
      fi
      ...
      echo "  Curl failed on attempt $attempt"
      if [[ $attempt -eq 3 ]]; then
        echo "  Error: Network error fetching crate info for $crate_name" >&2
        return 1
      fi
      sleep $((attempt * 2))
      continue
